...
#run  script for each set: Healthy-14, Healthy-42, Lesion-14, Lesion-42

#for Healthy:
zscore <- read.csv("14_clusters_HC_CT/skin_score.csv")
input_name <- "H_14_clusters_CellType"

zscore <- read.csv("42_clusters_HC_CT/skin_score.csv")
input_name <- "H_42_clusters_CellType"

#for Lesion:
zscore <- read.csv("14_clusters_AD_Lesion_CT/skin_score.csv")
input_name <- "LS_14_clusters_CellType"

zscore <- read.csv("42_clusters_AD_Lesion_CT/skin_score.csv")
...
scaled_mat_1 <- t(apply(pvalue_log_mat, 1, function(x) scale_func_1(x))) # across cells

#scaled_mat_2 <- t(apply(pvalue_log_mat, 1, function(x) scale_func_2(x))) #across cells

scaled_mat=scaled_mat_1
colnames(scaled_mat) <- colnames(pvalue_log_mat)


scaled_mat[which(rownames(scaled_mat)=="KRT5"),]
scaled_mat[which(rownames(scaled_mat)=="KRT1"),]
...
gene_loc <- read.table("/projects/abv/users/olivamx2/Projects/AD/reproduce_fanying/magma/src/NCBI37.3.gene.loc", stringsAsFactors=F,header=F)
head(gene_loc)
